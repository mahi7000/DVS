                    ...
                    if storage["file"].path == file.path and sink_line > storage["line"]:
                        ...
                        if not sanitized:
                            vulns.append(Vulnerability(
                                ...
                                file=file.path,
                                line=sink_line,
                                pattern=sink.description,
                                recommendation="Sanitize stored data before output using DOMPurify or similar." if sink.context == "html" else "Properly encode/escape stored data for the context.",
                                severity=Severity.HIGH,
                                confidence=calculate_confidence(sink_match.group()),
                                snippet=sink_match.group()[:200],
                                sanitized=sanitized
                            ))
