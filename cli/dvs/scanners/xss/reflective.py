...
#     def slice_has_any(slice_text: str, patterns: List[re.Pattern]) -> bool:
#         return any(pattern.search(slice_text) for pattern in patterns)
...
#                 for match in template_check["pattern"].finditer(content):
#                     idx = match.start()
...
#                         confidence=0.9,
#                         snippet=escape_for_display(content[max(0, idx - 50):idx + 50]),
#                         sanitized=False
#                     ))

#         # Server-side sink scanning
...
#                     has_source = slice_has_source(slice_text, server_sources)
...
#                             recommendation = "Never inject raw user input into CSS. Restrict to a safe allow-list (e.g., /^#[0-9a-f]{3,6}$/i for colors), or sanitize tokens (cssesc). Disallow url(javascript:), expression(), and external URLs."
...
#                             recommendation = "Ensure response is application/json and not embedded in HTML. Avoid reflecting untrusted JSON inside <script> without escaping."
#                             vulnerability_type = "Reflected JSON (check embedding)"
#                             severity = Severity.MEDIUM
#                         else:
#                             recommendation = "Escape/encode untrusted data before sending HTML (e.g., escapeHtml/validator.escape) or ensure the template auto-escapes."
...
#                             severity=severity,
#                             confidence=0.6 if sanitized else 0.9,
#                             snippet=escape_for_display(content[max(0, idx - 100):idx + 100]),
#                             sanitized=sanitized
#                         ))
